...
/*   Updated: 2020/11/23 13:45:44 by apavel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

int		ft_split_line(char **files, int fd, int bytes, char **line)
{
	char *tmp;
...
int		get_next_line(int fd, char **line)
{
	...
	if (fd == -1 || !line || !(buff = malloc(sizeof(char) * (32 + 1)))
	|| read(fd, buff, 0) == -1 || 32 <= 0)
		return (-1);
	while ((bytes = read(fd, buff, 32)) > 0)
	{
		buff[bytes] = '\0';
		if (files[fd] == NULL)
